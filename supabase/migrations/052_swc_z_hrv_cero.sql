-- =============================================================
-- 052 — La puerta SWC anulaba el z de HRV en vez de ponerlo en cero
--
-- El comentario de la mig 043 decia: "Si la desviacion no lo supera, el z se
-- fuerza a 0". El CASE no tenia ELSE, asi que devolvia NULL — que es el mismo
-- valor que usa "este cliente no tiene HRV".
--
-- POR QUE IMPORTA: aguas abajo, s_a_base elige formula segun z_hrv sea NULL:
--     con HRV y FC:  50 + 50 * clamp((z_hrv - z_rhr) / 4)
--     solo FC:       50 - 50 * clamp(z_rhr / 2)
-- Divisor 4 contra divisor 2. O sea que un dia con la HRV ESTABLE —donde la
-- puerta SWC deberia estar diciendo "no pasa nada, no muevas el score"— pasaba
-- a la rama de FC sola y quedaba con el DOBLE de sensibilidad a la frecuencia
-- cardiaca en reposo. Justo al reves de lo que la puerta busca.
--
-- Ejemplo concreto, dia con HRV estable y FC en reposo +1 SD:
--     con z_hrv = 0 (lo documentado):  50 + 50*(-1/4) = 37.5
--     con z_hrv = NULL (lo real):      50 - 50*(1/2)  = 25
-- 12.5 puntos de score, y en la direccion de asustar de mas.
--
-- Efecto secundario que tambien se corrige: la clasificacion de
-- estado_autonomico exige z_hrv IS NOT NULL, asi que en los dias estables el
-- cliente no se clasificaba nunca y caia a 'normal' por omision, no por
-- evaluacion. Con 0 se evalua de verdad (y sigue dando 'normal', pero porque
-- corresponde).
--
-- Es un CREATE OR REPLACE de mypump_calc_recuperacion con esa unica linea
-- cambiada. El resto es identico a la 043.
--
-- VERIFICACION: sobre un cliente con serie sintetica, comparar el score de un
-- dia con |ln_hrv - mu7| por debajo del umbral SWC antes y despues; y que
-- componentes->'autonomico' deje de saltearse en esos dias.
--
-- ROLLBACK: re-aplicar la mig 043.
-- =============================================================
