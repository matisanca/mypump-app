-- ============================================================
-- 043 — Motor de recuperación (score 0-100 diario)
-- ============================================================
-- Convierte las series crudas de mypump_salud_diaria en un score interpretable,
-- al estilo Whoop/Oura pero honesto con los datos que realmente tenemos.
--
-- POR QUÉ VIVE EN POSTGRES (y no en JS o en la Mac mini):
--   Tres consumidores necesitan EL MISMO número — la app del cliente, el panel
--   del coach y el centinela. En JS habría que reimplementarlo en Python y en
--   el panel; en Python la app no podría mostrar el score de hoy porque la mini
--   corre 21:30. Los datos ya están acá y las ventanas móviles son window
--   functions triviales. Precedente: mypump_get_metricas_coach (034).
--
-- POR QUÉ NO SE PERSISTE:
--   Todas las ventanas son ESTRICTAMENTE hacia atrás (7 PRECEDING AND 1
--   PRECEDING: excluyen el día evaluado). Recalcular da siempre lo mismo, así
--   que el score de un día pasado no se mueve. Materializarlo sería sincronizar
--   una caché sin ganancia.
--
-- ── FUNDAMENTO (lo que condiciona cada decisión de acá abajo) ──────────────
--
-- 1. El valor ABSOLUTO de HRV no significa nada entre personas: la diferencia
--    de baseline se explica por genética, edad, sexo y BMI. Solo importa la
--    desviación contra el propio histórico (Altini & Plews 2021, n≈28.000).
--    → Todo se mide en z-scores contra la media móvil de 7 días de la persona.
--
-- 2. Se usa Ln(HRV) porque rMSSD tiene distribución sesgada a la derecha; el
--    log normaliza y evita que un solo día alto domine la media de 7 días.
--    Es la convención de la literatura deportiva (LnrMSSD).
--
-- 3. HRV y FC en reposo NO son componentes independientes: son dos vistas del
--    mismo estado autonómico. Sumarlas por separado sería contar dos veces el
--    mismo estresor. Van fusionadas en un solo sub-score (componente A) usando
--    la matriz HRV × FC × CV, que es lo que permite distinguir fatiga de
--    maladaptación de saturación parasimpática.
--
-- 4. El CV (variabilidad de la variabilidad) sube ANTES de que caiga el
--    baseline: es el indicador temprano. Alto CV se asocia a alcohol, sueño
--    corto/irregular y menos actividad.
--
-- 5. SWC (smallest worthwhile change) = 0.5 × CV intra-sujeto. Por debajo de
--    eso, un cambio es ruido de medición, no señal. Con el SDNN del Apple
--    Watch (MAPE ~29% contra banda pectoral) el error de medición SUPERA la
--    señal diaria, así que su umbral se ensancha y su peso baja.
--
-- 6. FC en reposo es la métrica más robusta y universal: llega de casi todo
--    wearable y correlaciona MÁS con nivel de actividad (r=0.30) que la propia
--    HRV (r=0.21). Un score sin HRV pero con FC reposo + sueño es defendible.
--
-- 7. Ventaja propia: tenemos la carga REAL del gimnasio (series, RIR, tonelaje)
--    y el check subjetivo. Whoop y Oura no los tienen. No se trata de clonar su
--    número con peores datos, sino de usar los datos que ellos no ven.
--
-- 8. No hay evidencia de entrenamiento guiado por HRV en hipertrofia/fuerza
--    (toda es de resistencia, con muestras chicas). El score INFORMA, no manda.
-- ============================================================


-- ============================================================
-- Interna. Devuelve una fila por día del rango pedido.
-- ============================================================
...
cv AS (
  SELECT bl.*,
    CASE WHEN bl.hrv_n7 >= 3 AND bl.hrv_mu7 <> 0
         THEN ABS(bl.hrv_sd7 / bl.hrv_mu7) * 100 END AS cv7
  FROM bl
),
cvr AS (
...
z AS (
  SELECT c.*,
    CASE WHEN c.hrv_sd60 > 0 AND c.ln_hrv IS NOT NULL AND c.hrv_mu7 IS NOT NULL
              AND ABS(c.ln_hrv - c.hrv_mu7) >= COALESCE(0.005 * c.cv_ref, 0)
         THEN (c.ln_hrv - c.hrv_mu7) / c.hrv_sd60 END AS z_hrv,
...
SELECT
  ...
  jsonb_strip_nulls(jsonb_build_object(
    'autonomico', jsonb_build_object('score', ROUND(t.s_a), 'peso', t.w_a),
...
-- ============================================================
-- Wrapper del CLIENTE: por token, últimos N días.
-- ============================================================
CREATE OR REPLACE FUNCTION mypump_get_recuperacion(
  p_token TEXT,
...
BEGIN
  ...
  RETURN QUERY
    SELECT * FROM mypump_calc_recuperacion(
      v_cliente_id, v_hoy - GREATEST(1, LEAST(90, p_dias)) + 1, v_hoy);
END;
$$;

GRANT EXECUTE ON FUNCTION mypump_get_recuperacion(TEXT, INTEGER) TO anon, authenticated;


-- ============================================================
-- Wrapper del COACH: todos los clientes con app, último día + serie corta.
-- NUNCA anon: expone datos de todos los clientes.
-- ============================================================
CREATE OR REPLACE FUNCTION mypump_get_recuperacion_global(
  p_dias INTEGER DEFAULT 14
)
RETURNS TABLE (
  cliente_id TEXT, nombre TEXT, fecha DATE, score INTEGER, estado TEXT,
  banda TEXT, estado_autonomico TEXT, hrv_metrica TEXT, discordancia BOOLEAN,
  componentes JSONB, motivos JSONB
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  v_hoy := (NOW() AT TIME ZONE 'America/Argentina/Buenos_Aires')::DATE;
  RETURN QUERY
    SELECT c.cliente_id, c.nombre, r.fecha, r.score, r.estado, r.banda,
           r.estado_autonomico, r.hrv_metrica, r.discordancia, r.componentes, r.motivos
    FROM mypump_clientes c
    CROSS JOIN LATERAL mypump_calc_recuperacion(
      c.cliente_id, v_hoy - GREATEST(1, LEAST(90, p_dias)) + 1, v_hoy) r
    WHERE c.access_token_active = TRUE
    ORDER BY c.nombre, r.fecha;
END;
$$;

REVOKE ALL    ON FUNCTION mypump_get_recuperacion_global(INTEGER) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION mypump_get_recuperacion_global(INTEGER) TO authenticated, service_role;


-- ============================================================
-- Preferencia del cliente: poder APAGAR el número sin dejar de capturar.
-- Mitigación de ortosomnia (ansiedad inducida por el propio tracker): el dato
-- se sigue registrando y el coach lo sigue viendo; solo desaparece de la app.
-- ============================================================
ALTER TABLE mypump_cliente_prefs
  ADD COLUMN IF NOT EXISTS mostrar_recuperacion BOOLEAN NOT NULL DEFAULT TRUE;


-- ============================================================
-- ROLLBACK:
--   DROP FUNCTION IF EXISTS mypump_get_recuperacion_global(INTEGER);
--   DROP FUNCTION IF EXISTS mypump_get_recuperacion(TEXT, INTEGER);
--   DROP FUNCTION IF EXISTS mypump_calc_recuperacion(TEXT, DATE, DATE);
--   ALTER TABLE mypump_cliente_prefs DROP COLUMN IF EXISTS mostrar_recuperacion;
-- ============================================================
